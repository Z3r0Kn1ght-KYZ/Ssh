#!/bin/bash
# ═══════════════════════════════════════════════════════
#   NETFILES KYZ — Gestor de Servicios VPN/SSH
#   by Z3r0Kn1ght • @Z3r0Kn1ght
#   Ubuntu 22/24/25
# ═══════════════════════════════════════════════════════
...
echo -e "\033[0;32m✅ Acceso concedido\033[0m"
sleep 1
...
if [ ! -f /etc/sshzero/server_name ]; then
    mkdir -p /etc/sshzero
    ...
    INSTALL_NAME=${INSTALL_NAME:-"SSH ZERO"}
    echo "$INSTALL_NAME" > /etc/sshzero/server_name
    echo "$(date +%d-%m-%Y)" > /etc/sshzero/install_date
fi
...
INSTALL_DATE=$(cat /etc/sshzero/install_date 2>/dev/null || echo "N/A")
SRV_NAME=$(cat /etc/sshzero/server_name 2>/dev/null || echo "SSH ZERO")
...
echo -e "  ${YELLOW}CREADOR DEL SCRIPT${NC}       : ${PURPLE}@Z3r0Kn1ght ❴KYZ❵${NC}"
echo -e "  ${GREEN}BIENVENIDO DE NUEVO!${NC}"
echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "  Teclee ${YELLOW}menu${NC} para ver el MENU KYZ"
echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo ""
MOTDSCRIPT
...
instalar_ws() {
    ...
    BANNER_MSG=${BANNER_MSG:-"SSH ZERO by Z3r0Kn1ght"}
    echo ""; sep
...
EOF
                mkdir -p /etc/sshzero
                echo "$DOMAIN" > /etc/sshzero/v2ray_domain
...
if tls == "tls":
    domain = open('/etc/sshzero/v2ray_domain').read().strip() if os.path.exists('/etc/sshzero/v2ray_domain') else ''
...
" 2>/dev/null
                ...
                VDOMAIN=$(cat /etc/sshzero/v2ray_domain 2>/dev/null || hostname -I | awk '{print $1}')
                python3 - << PYEOF
...
instalar_motd() {
    ...
    echo "$INSTALL_DATE" > /etc/sshzero/install_date
    echo "$SRV_NAME" > /etc/sshzero/server_name
...
actualizar_script() {
    ...
    echo -e "  ${C}Descargando ultima version...${NC}"
    wget -q -O /usr/local/bin/menu "https://raw.githubusercontent.com/Z3r0Kn1ght-KYZ/Ssh/refs/heads/main/ssh-manager.sh?$(date +%s)"
    chmod +x /usr/local/bin/menu
    mkdir -p /etc/sshzero
    touch /etc/sshzero/.licensed
...
menu_dropbear() {
    while true; do
        ...
        DB_PORT=$(cat /etc/sshzero/dropbear_port 2>/dev/null || echo "444")
  ...
  ${C}Instalando Dropbear...${NC}"
                ...
                DB_PORT=${DB_PORT:-444}
                mkdir -p /etc/sshzero
                echo "$DB_PORT" > /etc/sshzero/dropbear_port
                # Configurar
...
EOF
            ...
            5)
                ...
                echo "$NEW_PORT" > /etc/sshzero/dropbear_port
    ...
    while true; do
        ...
        case $OPT in
            1)
                ...
                echo -e "  Ejemplo:"
                echo -e "  ╔══════════════════════════════════╗"
                echo -e "  ║   SERVIDOR PRIVADO - SSH ZERO ║"
                echo -e "  ╚══════════════════════════════════╝"
                echo ""; sep
...
menu_herramientas() {
    while true; do
        ...
        printf " ${NEON}◈${NC} ${W}Dropbear${NC}  %-12b ${NEON}◈${NC} ${W}KYZUDPv1${NC}    %b\n" "$(status_service dropbear)" "$(status_service hysteria-server)"
        echo ""; sep
...
menu_principal() {
    while true; do
        ...
        sep
        WS_PORT=$(cat /etc/sshzero/ws_port 2>/dev/null || echo "80")
        DB_PORT=$(cat /etc/sshzero/dropbear_port 2>/dev/null || echo "444")
        C1="" C2=""
        ...
        systemctl is-active --quiet hysteria-server 2>/dev/null && { [ -z "$C1" ] && C1="${NEON}◈${NC} ${W}KYZUDPv1:36712${NC} ${NEON}◆ ON${NC}" || { [ -z "$C2" ] && C2="${NEON}◈${NC} ${W}KYZUDPv1:36712${NC} ${NEON}◆ ON${NC}" || { echo -e " $C1    $C2"; C1="${NEON}◈${NC} ${W}KYZUDPv1:36712${NC} ${NEON}◆ ON${NC}"; C2=""; }; }; }
...
wget -q -O /usr/local/bin/menu "https://raw.githubusercontent.com/Z3r0Kn1ght-KYZ/Ssh/refs/heads/main/ssh-manager.sh"
chmod +x /usr/local/bin/menu
